...
const TEMPLATES = [
  ...
  },
  {
    ...
    roi: 'Prevents degraded application performance. Saves 3 hrs/month.',
    steps: ['Capture packet loss metrics from monitoring agent', 'Run traceroute to identify path hop drop', 'Check MTU settings on affected interfaces', 'AI compares MTU values across network path', 'If mismatch found: apply MTU correction value', 'Verify packet loss returns to < 0.1%', 'Document network topology change'],
  },
  {
    id: 'TPL-051', category: 'Network', industry: ['All'],
    title: 'DNS Resolution Failure — Automatic Failover',
    severity: 'Critical', confidence: '97%',
    ...
    roi: 'Prevents 100% service impact. Reduces MTTR from 30 min to 5 min.',
    steps: ['Detect DNS timeout from health check agent', 'Verify primary DNS server is reachable via ICMP', 'Check DNS service status on primary server', 'If server down: update resolv.conf to point to secondary', 'Flush DNS cache on affected systems', 'Verify resolution succeeds on critical domains'],
  },
  {
    id: 'TPL-052', category: 'Network', industry: ['All'],
    title: 'BGP Route Flapping & Path Remediation',
    severity: 'High', confidence: '94%',
    estimated_resolution_mins: 10, monthly_occurrences: 4,
    tags: ['network', 'bgp', 'routing'],
    description: 'Detects BGP neighbor instability and dampens flapping routes to maintain predictable network latency.',
    roi: '4x/month. Eliminates inter-datacenter route flap outages.',
    steps: ['Receive syslog trigger for BGP neighbor state change', 'Query BGP routing table via SNMP/SSH', 'Identify flapping prefix or unstable peer', 'Apply route dampening policy to unstable neighbor', 'Verify secondary path handles traffic', 'Alert NOC team'],
  },
  {
