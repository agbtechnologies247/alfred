...
pub async fn execute_workflow(
    ...
    if let Err((_status, err_json)) =
        require_permission(&user, &["super_admin", "sr_engineer", "engineer"])
    {
    ...
    let graph_json = payload.get("graph").cloned().unwrap_or_else(|| {
        json!({
            "id": workflow_id,
    ...
    match state
        .workflow_engine
        .execute_graph(&graph_json, json!({}))
        .await
    {
        Ok(()) => Json(
            json!({ "success": true, "message": "Workflow executed successfully", "workflow_id": workflow_id }),
        ),
