...
impl SentimentAnalyzer {
    ...
    pub fn analyze(person_id: Uuid, text: &str) -> SentimentResult {
        ...
        let stress_keywords = [
            ("blocked", 0.25), ("waiting", 0.15), ("frustrated", 0.40),
            ("angry", 0.50), ("stuck", 0.30), ("overloaded", 0.45),
            ("overwhelmed", 0.50), ("impossible", 0.35), ("broken", 0.25),
            ("failed", 0.20), ("down", 0.15), ("critical", 0.20),
            ("urgent", 0.20), ("escalat", 0.30), ("customer is angry", 0.45),
            ("can't", 0.15), ("no support", 0.35), ("too many", 0.25),
            ("exhausted", 0.55), ("burnout", 0.60), ("tired", 0.30),
            ("late night", 0.25), ("weekend work", 0.30), ("stress", 0.40),
            ("deadline", 0.20), ("pressure", 0.30), ("conflict", 0.35),
        ];
        ...
        let positive_keywords = [
            ("completed", -0.15), ("resolved", -0.15), ("fixed", -0.10),
            ("great", -0.10), ("good progress", -0.15), ("on track", -0.10),
            ("ahead of schedule", -0.20), ("happy", -0.15), ("excited", -0.10),
            ("thank", -0.05), ("helped", -0.10), ("learned", -0.05),
        ];
...
mod tests {
    ...
    fn test_stressed_message() {
        ...
        assert!(matches!(result.burnout_risk, BurnoutRisk::High | BurnoutRisk::Critical | BurnoutRisk::Moderate));
    ...
    fn test_positive_message() {
        ...
        assert!(matches!(result.emotion, Emotion::Positive | Emotion::Neutral));
    }
    ...
    fn test_burnout_message() {
        ...
        assert!(matches!(result.burnout_risk, BurnoutRisk::Critical | BurnoutRisk::High));
    }
