...
impl PacketAnalyzer {
    ...
    pub fn analyze_telemetry(
        ...
        if packet_loss > 0.0 {
            tracing::warn!(
                "Packet Engine: Analyzing packet drops ({:.2}%) on host: {}",
                packet_loss,
                host
            );
            ...
            if packet_loss > 8.0 && cpu < 80.0 {
                tracing::error!(
                    "Packet Engine Detected: Potential MTU mismatch path drop on {}",
                    host
                );
                let event = AlfredEvent::AiAnalysisCompleted {
                    incident_id: format!("packet-anomaly-{}", host),
                    tags: vec![
                        "MTU_Mismatch".to_string(),
                        "Layer_3".to_string(),
                        host.to_string(),
                    ],
                    confidence: 0.96,
...
pub fn init_packet_engine(event_bus: EventBus) {
    ...
    tokio::spawn(async move {
        let socket = match UdpSocket::bind("127.0.0.1:9999").await {
            ...
            Err(e) => {
                tracing::error!(
                    "Packet Engine: Failed to bind UDP listener on port 9999: {}",
                    e
                );
                return;
            }
        ...
        loop {
            match socket.recv_from(&mut buf).await {
                Ok((size, addr)) => {
                    tracing::info!(
                        "Packet Engine: Intercepted raw socket frame of {} bytes from {}",
                        size,
                        addr
                    );
                    ...
                    if size > 1500 {
                        tracing::warn!(
                            "Packet Engine Detected: Jumbo frame (size={}) intercepted from {}",
                            size,
                            addr
                        );
                        let event = AlfredEvent::AiAnalysisCompleted {
...
mod tests {
    ...
    async fn test_packet_engine_jumbo_frame() {
        ...
        sender
            .send_to(&payload, "127.0.0.1:9999")
        ...
        for _ in 0..10 {
            if let Ok(event) = rx.try_recv() {
                if let AlfredEvent::AiAnalysisCompleted {
                    incident_id, tags, ..
                } = event
                {
                    if incident_id.contains("packet-jumbo-")
                        && tags.contains(&"Jumbo_Frame".to_string())
                    {
                        success = true;
                        break;
                    }
